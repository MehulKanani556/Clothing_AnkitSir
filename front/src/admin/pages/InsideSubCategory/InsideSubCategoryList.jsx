...
const InsideSubCategoryList = () => {
  ...
  const { insideSubCategories, loading } = useSelector((state) => state.category);
  ...
  const handleDelete = (id) => {
    if (window.confirm('Are you sure you want to delete this inside sub category?')) {
      dispatch(deleteInsideSubCategory(id));
    }
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-black text-mainText tracking-tight">Detailed Categories</h2>
          <p className="text-lightText text-sm font-medium">View and manage your detailed sub-categories here.</p>
        </div>
        <div className="flex gap-3">
          <button 
            onClick={() => dispatch(fetchInsideSubCategories())}
          ...
          >
            <MdAdd size={20} />
            <span>New Inside Sub Category</span>
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-primary p-5 rounded-none border border-primary flex items-center justify-between shadow-xl shadow-primary/10">
          <div>
            ...
            <p className="text-3xl font-black text-white leading-tight">{insideSubCategories.length}</p>
          </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-mainBG/50 border-b border-border">
                ...
                <th className="px-8 py-5 text-[10px] font-black text-lightText uppercase tracking-[0.2em]">Hierarchy</th>
            ...
            <tbody className="divide-y divide-border/50">
              {loading && insideSubCategories.length === 0 ? (
                ...
                <tr>
                  <td colSpan="6" className="px-8 py-16 text-center text-lightText font-bold text-sm italic">No categories found matching your search.</td>
                </tr>
              ) : (
                paginatedCategories.map((category) => (
                  <tr key={category._id} className="hover:bg-mainBG/30 transition-colors group">
                    ...
                    <td className="px-8 py-5">
                      <span className="font-black text-mainText text-sm tracking-tight">{category.insideSubCategoryName}</span>
                    </td>
                    <td className="px-8 py-5">
                      <div className="flex flex-col gap-1">
                        <span className="text-[11px] font-black text-primary uppercase tracking-widest bg-primary/5 px-3 py-1 rounded-none border border-primary/10 w-fit">
                          {category.subCategoryId?.subCategoryName || 'N/A'}
                        </span>
                        <span className="text-[9px] text-lightText font-bold uppercase tracking-tight ml-1">
                          Part of {category.categoryId?.categoryName || 'General'}
                        </span>
                      </div>
                    </td>
...
export default InsideSubCategoryList;
