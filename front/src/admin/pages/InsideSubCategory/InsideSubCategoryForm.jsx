      ...
      <div className="px-10 py-6 border-b border-border flex justify-between items-center bg-mainBG/30">
        <div>
          <h3 className="font-black text-mainText text-xl tracking-tight">
            {initialValues ? 'Edit Detailed Category' : 'Add New Detailed Category'}
          </h3>
      ...
      <form onSubmit={formik.handleSubmit} className="p-10 space-y-8">
              ...
              {preview ? (
                ...
                <div className="flex flex-col items-center gap-4 py-8">
                  ...
                  <div className="text-center">
                    <p className="text-sm font-black text-mainText tracking-tight">Upload Category Media</p>
                    <p className="text-[10px] text-lightText font-black uppercase tracking-[0.1em] mt-1.5">Drag & Drop or Click</p>
                  </div>
                </div>
              )}
              <input
        ...
        <div className="grid grid-cols-1 gap-8">
          <div className="space-y-2">
            <label htmlFor="insideSubCategoryName" className="text-[11px] font-black text-mainText uppercase tracking-widest ml-1 opacity-70">
            ...
            <input
              ...
              className={`w-full px-6 py-4 rounded-none border transition-all outline-none text-sm font-black tracking-tight ${formik.touched.insideSubCategoryName && formik.errors.insideSubCategoryName
            ...
            {formik.touched.insideSubCategoryName && formik.errors.insideSubCategoryName && (
              <p className="text-[10px] font-black text-red-500 ml-1 uppercase tracking-widest mt-1">{formik.errors.insideSubCategoryName}</p>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 gap-6">
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                <label htmlFor="subCategoryId" className="text-[11px] font-black text-mainText uppercase tracking-widest ml-1 opacity-70">
                  Sub Category
                </label>
                <select
                  id="subCategoryId"
                  ...
                  className={`w-full px-6 py-4 rounded-none border transition-all outline-none text-sm font-black tracking-tight appearance-none bg-mainBG/10 ${formik.touched.subCategoryId && formik.errors.subCategoryId
                    ? 'border-red-300 focus:border-red-500 bg-red-50/10'
                    : 'border-border focus:border-primary focus:ring-8 focus:ring-primary/5'
                    } ${!formik.values.categoryId ? 'bg-mainBG cursor-not-allowed opacity-60' : ''}`}
                  {...formik.getFieldProps('subCategoryId')}
                >
                  <option value="" className="font-black">Select Sub</option>
                  {filteredSubCategories.map((subCat) => (
                    <option key={subCat._id} value={subCat._id} className="font-black">
                      {subCat.subCategoryName}
                    </option>
                  ))}
                </select>
                {formik.touched.subCategoryId && formik.errors.subCategoryId && (
                  <p className="text-[10px] font-black text-red-500 ml-1 uppercase tracking-widest mt-1">{formik.errors.subCategoryId}</p>
                )}
              </div>
            </div>
          </div>
