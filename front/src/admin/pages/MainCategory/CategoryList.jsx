...
const CategoryList = () => {
  ...
  const handleCreate = (values) => {
    ...
    if (values.mainCategoryImage) {
      formData.append('mainCategoryImage', values.mainCategoryImage);
    }

    dispatch(createMainCategory(formData)).then((res) => {
  ...
  const handleUpdate = (values) => {
    ...
    dispatch(updateMainCategory({ id: editingCategory._id, data: formData })).then((res) => {
      if (!res.error) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-black text-mainText tracking-tight">Main Categories</h2>
          <p className="text-lightText text-sm font-medium">View and manage your main store categories here.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-primary p-6 rounded-none border border-primary/10 flex items-center justify-between shadow-xl shadow-primary/20 group overflow-hidden relative">
            <div className="relative z-10">
                ...
                <p className="text-3xl font-black text-white leading-tight">{mainCategories.length}</p>
            </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-border/50">
              {loading && mainCategories.length === 0 ? (
                ...
                <tr>
                    <td colSpan="5" className="px-8 py-20 text-center font-black uppercase tracking-widest text-xs text-lightText opacity-50">
                ...
                paginatedCategories.map((category) => (
                  <tr key={category._id} className="hover:bg-mainBG/30 transition-colors group">
                    <td className="px-8 py-5 whitespace-nowrap">
                      <div className="w-14 h-14 rounded-none bg-mainBG overflow-hidden border border-border group-hover:border-primary/20 transition-all duration-500 shadow-sm">
                        {category.mainCategoryImage ? (
                          <img 
                            src={category.mainCategoryImage.startsWith('http') ? category.mainCategoryImage : `http://localhost:8000/${category.mainCategoryImage}`} 
                            alt={category.mainCategoryName}
                            className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110"
                            onError={(e) => { e.target.src = 'https://ui-avatars.com/api/?name=' + category.mainCategoryName }}
                          />
                        ) : (
                    ...
                    <td className="px-8 py-5 whitespace-nowrap">
                      <span className="text-sm font-black text-mainText tracking-tight">{category.mainCategoryName}</span>
                    </td>
                    <td className="px-8 py-5 whitespace-nowrap">
